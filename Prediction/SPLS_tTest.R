t.test.select.spls <- function(expTable, ctrlTable, p_value) {
  ...
  for(i in 1:ncol(exp)){
    ...
    for(j in 1:ncol(ctrl)){
      ...
      TRAINCV <- spls(train_matrix, phenotypes, eta = seq(0.1,0.9,0.1), K=2)
      
      prediction <- predict.spls(TRAINCV, test_matrix)
